...
struct FingerprintView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            // Header
            HStack {
                Text(NSLocalizedString("SECURITY VERIFICATION", comment: ""))
            ...
            VStack(alignment: .leading, spacing: 16) {
                // Peer info
                let peerNickname = viewModel.meshService.getPeerNicknames()[peerID] ?? NSLocalizedString("Unknown", comment: "")
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text(NSLocalizedString("THEIR FINGERPRINT:", comment: ""))
                    ...
                    if let fingerprint = viewModel.getFingerprint(for: peerID) {
                        ...
                        Text(NSLocalizedString("not available - handshake in progress", comment: ""))
                            .font(.system(size: 14, design: .monospaced))
                ...
                // My fingerprint
                VStack(alignment: .leading, spacing: 8) {
                    Text(NSLocalizedString("YOUR FINGERPRINT:", comment: ""))
                        .font(.system(size: 12, weight: .bold, design: .monospaced))
                        .foregroundColor(textColor.opacity(0.7))
                ...
                if encryptionStatus == .noiseSecured || encryptionStatus == .noiseVerified {
                    ...
                    VStack(spacing: 12) {
                        Text(isVerified ? "✓ " + NSLocalizedString("VERIFIED", comment: "") : "⚠️ " + NSLocalizedString("NOT VERIFIED", comment: ""))
                            .font(.system(size: 14, weight: .bold, design: .monospaced))
                            .foregroundColor(isVerified ? Color.green : Color.orange)
                            .frame(maxWidth: .infinity)
                        
                        Text(isVerified ? 
                             NSLocalizedString("you have verified this person's identity.", comment: "") :
                             String(format: NSLocalizedString("compare these fingerprints with %@ using a secure channel.", comment: ""), peerNickname))
                            .font(.system(size: 12, design: .monospaced))
                            .foregroundColor(textColor.opacity(0.7))
                        ...
                        if !isVerified {
                            Button(action: {
                                ...
                                Text(NSLocalizedString("MARK AS VERIFIED", comment: ""))
                                    .font(.system(size: 14, weight: .bold, design: .monospaced))
