...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            selectedMessageSender.map { "@\($0)" } ?? NSLocalizedString("Actions", comment: ""),
            isPresented: $showMessageActions,
            titleVisibility: .visible
        ) {
            Button(NSLocalizedString("private message", comment: "")) {
            ...
            Button(NSLocalizedString("hug", comment: "")) {
                if let sender = selectedMessageSender {
                    viewModel.sendMessage("/hug @\(sender)")
                }
            }
            
            Button(NSLocalizedString("slap", comment: "")) {
                if let sender = selectedMessageSender {
    ...
    private var inputView: some View {
        VStack(spacing: 0) {
            ...
            TextField(NSLocalizedString("type a message...", comment: ""), text: $messageText)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if !viewModel.connectedPeers.isEmpty {
                            HStack(spacing: 4) {
                                ...
                                Text(NSLocalizedString("PEOPLE", comment: ""))
                        ...
                        if viewModel.connectedPeers.isEmpty {
                            Text(NSLocalizedString("nobody around...", comment: ""))
                                .font(.system(size: 14, design: .monospaced))
                                .foregroundColor(secondaryTextColor)
                                .padding(.horizontal)
                        } else {
                        ...
                        ForEach(peerData) { peer in
                            HStack(spacing: 8) {
                                ...
                                if peer.isMe {
                                    HStack {
                                        Text(peer.displayName + " (" + NSLocalizedString("you", comment: "") + ")")
                                            .font(.system(size: 14, design: .monospaced))
                                            .foregroundColor(textColor)
